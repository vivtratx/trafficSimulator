 ...
 * output: none
 *
 * frees the given Queue pointer.  Also, you should remember to free the
 * elements in the linked list!
 */
void freeQueue(Queue *pq) {
    while (!isEmpty(pq)) dequeue(pq);
    free(pq);
}
 ...
 *
 * Returns the queueType stored at the front of the Queue.  It does not remove
 ...
 * output: a pointer to queueType
 *
 * Dequeues and returns the queueType stored at the front of the Queue.  It does
 * not free the dequeue-ed element.
 */
 ...
 * output: a pointer to queueType
 *
 * Returns the queueType stored at the front of the Queue.  It does not dequeue
 * or free the front element.
 */
...
void mergeQueues(Queue *to, Queue *from) {
    while (!isEmpty(from)) enqueue(to, dequeue(from));
}
...
bool isEmpty(Queue *pq) {
    if (pq->qFront == NULL && pq->qRear == NULL) {
        ...
        fprintf(stderr,
                "isEmpty: Queue had inconsistent values for front and rear.\n");
        exit(-1);
    }
    return false;
}
 ...
 */
int getNumElements(Queue *pq) { return pq->numElements; }
