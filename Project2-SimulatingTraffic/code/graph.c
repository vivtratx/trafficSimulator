...
bool isBetterPath( int ij, int ik, int kj);
void ssspBellmanFord(Graph* g, int source);

/* createGraph
 * input: an int representing the maximum number of vertices needed in your
 * graph output: a pointer to a Graph (this is malloc-ed so be sure to call
 * freeGraph eventually)
 *
 * Creates a new empty Graph and returns a pointer to it.  The maximum number of
 * vertices is set to capacity.
 */
...
void addVertex(Graph* g, graphType p) {
    ...
    if (g->numVertices == g->capacity) {
        printf(
            "ERROR - addVertex - Attempt to add more vertices than the "
    ...
    if (i != -1) {
        printf(
            "WARNING - addVertex - Attempt to add same graphType to graph "
            "multiple times\n");
        return;
    }
 ...
 */
bool isVertex(Graph* g, graphType p) { return getIndex(g, p) != -1; }

/* getVertexByIndex
 * input: a pointer to a Graph, an int, a pointer to a graphType
 * output: a bool
 *
 * finds index-th vertex added to the graph and inserts into next.  It returns
 * true if this search was successful and false otherwise.
 */
bool getVertexByIndex(Graph* g, int index, graphType* next) {
    if (index >= g->numVertices) return false;
 ...
 * output: none
 *
 * Sets the specified edge in the graph to have a cost equal to value
 */
void setEdge(Graph* g, graphType p1, graphType p2, int value) {
    ...
    g->changed = true;
    g->adjacencyMatrix[i][j] = value; /* set cost/length of edge */
}
 ...
 * output: a int
 *
 * Returns the value for the specified edge in the graph or -1 if the
...
/* getSuccessor
 * input: a pointer to a Graph, a graphType 'start', a pointer to a graphType
 * 'pnext' output: a bool and the next successor of start is stored at 'pnext'
 *
 * Finds the next node reachable from 'start'.  It stores the destination of
 * this edge into location pointed to by 'pnext'. If a successor exists it
...
bool getSuccessor(Graph* g, graphType start, graphType* pnext) {
    ...
    if (i == -1) {
        ...
        printf(
            "ERROR - getSuccessor - Attempt to call getSuccessor with NULL "
            "pnext\n");
        exit(-1);
    }
...
/* getPredecessor
 * input: a pointer to a Graph, a graphType 'start', a pointer to a graphType
 * 'pprev' output: a bool and the next node with an edge going to start is
 * stored at 'pprev'
 *
 * Finds the next node which has an edge going to 'start'.  It stores the
 * destination of this edge into location pointed to by 'pprev'. If a
 * predecessor exists it returns true and false otherwise.
 */
bool getPredecessor(Graph* g, graphType start, graphType* pprev) {
    ...
    if (j == -1) {
        ...
        printf(
            "ERROR - getPredecessor - Attempt to call getSuccessor with NULL "
            "pprev\n");
        exit(-1);
    }
...
void setVisited(Graph* g, graphType p, bool value) {
    ...
    if (i == -1) {
        printf(
            "WARNING - setVisited - Attempt to set visited for a graphType not "
            "in graph \n");
        addVertex(g, p);
        i = getIndex(g, p);
    }
...
bool getVisited(Graph* g, graphType p) {
    ...
    if (i == -1) {
        printf(
            "WARNING - getVisited - Attempt to get visited for a graphType not "
            "in graph \n");
        addVertex(g, p);
        i = getIndex(g, p);
    }
...
void setDistance(Graph* g, graphType from, graphType to, int dist) {
    ...
    if (i == -1) {
        printf(
            "WARNING - setDistance - Attempt to set distance for a graphType "
            "not in graph \n");
        addVertex(g, from);
        i = getIndex(g, from);
    }
...
int getDistance(Graph* g, graphType from, graphType to) {
    ...
    if (i == -1) {
        printf(
            "WARNING - getDistance - Attempt to get distance for a graphType "
            "not in graph \n");
        addVertex(g, from);
        i = getIndex(g, from);
    }
 ...
 * output: none
 *
 * Sets the specified edgeData in the graph to contain the edgeType value (i.e. a Road)
 */
 ...
 * output: an edgeType
 *
 * Finds and returns the specified edgeData (i.e. a Road) for this edge in the graph
 */
 ...
 * input: a pointer to a Graph, a graphType 'to', a graphType 'from', a pointer
 * to a graphType 'pnext' output: a bool
 *
 * Returns false if no path exists.
 * Otherwise, returns true and fills pnext with next graph element to visit on a
 * shortest path from 'from' to 'to'.
 */
bool getNextOnShortestPath(Graph* g, graphType from, graphType to,
                           graphType* pnext) {
    int i = getIndex(g, from);
    int j = getIndex(g, to);
...
void updatePathsArray(Graph* g) {
    ...
    for (i = 0; i < g->capacity; i++) g->distance[i][i] = 0;
    ...
    for (i = 0; i < g->capacity; i++) g->pathsArray[i][i] = i;

    /* Floyd-Warshall algorithm */
    for (k = 0; k < g->capacity; k++)
        for (i = 0; i < g->capacity; i++)
            for (j = 0; j < g->capacity; j++)
                if (isBetterPath(g->distance[i][j], g->distance[i][k],g->distance[k][j])) {
                    g->distance[i][j] = g->distance[i][k] + g->distance[k][j];
                    /* update the stored shortest path */
                    g->pathsArray[i][j] = g->pathsArray[i][k];
                }
...
/* isBetterPath
 * INTERNAL USE ONLY
 * Check if ik+jk is better than ij
 * #s are negative if no path exists
 */
bool isBetterPath(int ij, int ik, int kj) {
    return ik >= 0 && kj >= 0 && (ij < 0 || ik + kj < ij);
}
...
void ssspBellmanFord(Graph* g, int source) {
    ...
    for (k = 0; k < g->capacity; k++)
        for (i = 0; i < g->capacity; i++)
            for (j = 0; j < g->capacity; j++)
                if (g->adjacencyMatrix[i][j] > 0 &&
                    g->distance[source][i] > 0 &&
                    (g->distance[source][j] <= 0 ||
                     g->distance[source][i] + g->adjacencyMatrix[i][j] <
                         g->distance[source][j])) {
                    g->distance[source][j] =
                        g->distance[source][i] + g->adjacencyMatrix[i][j];
